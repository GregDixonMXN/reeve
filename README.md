...
#    - a protected config outside Axiom's writable workspaces

# 5. Configure OpenAI for hybrid/cloud mode (optional)
export AXIOM_OPENAI_API_KEY="your-platform-api-key"
...
key in the environment; Axiom never sends it to the frontend.

## Configuration model

Axiom finds its base config from `AXIOM_CONFIG`, the current directory,
`$XDG_CONFIG_HOME/axiom`, or the executable and its parent directories. An
explicit `AXIOM_CONFIG` must name a readable file; missing or unreadable paths
fail startup instead of silently loading defaults. Axiom then loads the adjacent
`axiom.local.toml` and finally applies environment overrides. Relative
...
`make setup` installs the Python dependencies in `~/.axiom/venv`, outside the
workspace directories that agent tools may modify. Keep `tools.python_path`
outside every configured writable/allowed directory; Axiom rejects an embedded
...
- `AXIOM_OPENAI_API_KEY` (preferred for Axiom)
- `OPENAI_API_KEY`
- `AXIOM_ANTHROPIC_KEY`
- `ANTHROPIC_API_KEY`
- `AXIOM_GEMINI_KEY`
...
The Axiom-specific OpenAI variable wins when both OpenAI variables are set.
Never commit a key to `axiom.toml`, expose it to the frontend, or paste it into
...
children but some limits are per process or per user, so Axiom remains a
