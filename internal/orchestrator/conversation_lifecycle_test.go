...
func TestConversationLifecycleRestoresTitleAndMessages(t *testing.T) {
	...
	response, err := orch.AgentLoop("conv-restore", "run-1", "Repair conversation restoration in Axiom", func(event LoopEvent) {
	...
	if summaries[0].Title != "Repair conversation restoration in Axiom" || summaries[0].MessageCount != 2 {
	...
	if conversation.Messages[0].Content != "Repair conversation restoration in Axiom" || conversation.Messages[1].Content != "Lifecycle repaired" {
...
func TestDeriveConversationTitle(t *testing.T) {
	if got := deriveConversationTitle("  Analyze   the Axiom project\ncarefully  "); got != "Analyze the Axiom project carefully" {
