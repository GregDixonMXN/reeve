...
// embedded Reeve helper scripts so source code never needs shell quoting.
...
func (s *Sandbox) executeParts(ctx context.Context, policyCommand string, parts []string, dir string, stdin io.Reader, isolationRoots []string) *ExecuteResult {
	...
	if !s.cfg.Enabled {
		return &ExecuteResult{Error: "Code execution is disabled. Set security.sandbox.enabled = true in reeve.toml"}
	}
	...
	if s.cfg.RequireOSIsolation {
		...
		tempHome, err := os.MkdirTemp("", "reeve-sandbox-")
		if err != nil {
